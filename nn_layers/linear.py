...
class LinearLayer(nn.Module):
    ...
    def __init__(
        ...
        """
        super().__init__()
        self.w = nn.Parameter(torch.randn(dim_in, dim_out, generator=generator, dtype=torch.float, requires_grad=True))
        self.b = nn.Parameter(torch.randn(dim_out, generator=generator, dtype=torch.float, requires_grad=True))
