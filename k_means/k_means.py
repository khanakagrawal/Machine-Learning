...
def calculate_centers(
    ...
    """
    sums = np.zeros((num_centers, data.shape[1]))
    counts = np.zeros(num_centers)

    np.add.at(sums, classifications, data)
    np.add.at(counts, classifications, 1)
...
def lloyd_algorithm(
    ...
    prev_centers = data[:num_centers].copy()
    classifications = cluster_data(data, prev_centers)
    errors.append(calculate_error(data, prev_centers))
    ...
    while True:
        ...
        if np.max(np.abs(centers - prev_centers)) <= epsilon:
            break
